...
applyType sub t@(TUnknown () n) = fromMaybe t (Map.lookup n sub)
applyType _ t = t
  ...
  | otherwise = 
      ...
      case (t1, t2) of
        ...
        (TArrow _ t1 t2, TArrow _ t1' t2') -> do
          s1 <- unify t1 t1'
          s2 <- unify (applyType s1 t2) (applyType s1 t2')
          return (s1 <> s2)
...
envFrom = foldlM aux (mempty, mempty)
  where
    aux jojo@(env,sub) dec = case dec of
      (DecAnno _ n_ t_) ->
        let
          n = void n_
          t = void t_
...
applyTypeM :: (Functor f) => Subst -> f (a, Type ()) -> f (a, Type ())
applyTypeM sub = fmap2nd (applyType sub)
...
inferExpr (env,sub) expr = return (s1, addType t expr) -- TODO: 
  where
...
inferDecs (env, sub) = foldlM aux (sub, [])
  where
    ...
    aux (s0, decs) (Dec a name args body) = do
      ...
      let finalBody = applyTypeM s5 body'
      let funcType' = applyType (s5 <> s4 <> s210) funcType
      let finalArgs = map (applyTypeM (s5 <> s4 <> s210)) argWithTypes
      let finalName = addType (TUnit ()) name
      let finalSub = s5 <> s4 <> s210
      let finalDec = Dec (a, funcType') finalName finalArgs finalBody
      return (finalSub, finalDec : (map (applyTypeM finalSub) decs))
